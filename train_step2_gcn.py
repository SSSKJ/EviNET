...
from models import GCN, GraphModel, InvLinear
from data_utils import get_measures, eval_aurc, eval_acc
from utils.calculation_tools import cal_logit, edl_loss, get_scoreNN_gcn
...
deepset.load_state_dict(torch.load(f'./models/{args.dataset}/{args.prefix}/{args.prefix}_h{args.hidden_channels}_g{args.gamma}_lr{args.lr}_d{args.dropout}_a{args.aggr}_deepset.pth'))
encoder = GraphModel(dataset_ind.x.size(1), num_features, args).to(device)
encoder.load_state_dict(torch.load(f'./models/{args.dataset}/{args.prefix}/{args.prefix}_h{args.hidden_channels}_g{args.gamma}_lr{args.lr}_d{args.dropout}_a{args.aggr}_encoder.pth'))
...
for run in range(args.runs):
    ...
    Beta2E = [GCN(args.hidden_channels * 2, args.hidden_channels * 2, 1, 3, 0.4).to(device) for _ in range(c+1)]
