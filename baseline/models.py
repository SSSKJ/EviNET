...
class CRL(nn.Module):
    ...
    def loss_compute(self, dataset_ind, dataset_ood, criterion, device, args):
        ...
        train_idx = dataset_ind.splits['train']
        idx = torch.arange(train_idx.size(0))

        if self.correctness_history is None:
            self.correctness_history = crl_utils.History(len(idx))
...
class SGCN(nn.Module):
    def __init__(self, d, c, args):
        ...
        self.params["use_kernel"] = bool(args.gkde_use_kernel) # False
    ...
    def loss_compute(self, dataset_ind, dataset_ood, criterion, device, args):
        ...
        if self.params["teacher_training"]:
            ...
            try:
                ...
                print(sum(prediction.soft <= 0))
                print(torch.isnan(prediction.soft).sum().item())
                mask = (prediction.soft <= 0) | torch.isnan(prediction.soft)
                print(prediction.soft[mask.any(dim=1)])
                print(prediction.alpha[mask.any(dim=1)])
                exit(1)
        ...
        # print(losses['BR'])
        # print(losses['KL_prior'])
        # print(losses['KL_teacher'])
...
class EnergyModel(nn.Module):
    ...
    def loss_compute(self, dataset_ind, dataset_ood, criterion, device, args):
        ...
        train_in_idx, _ = dataset_ind.splits['train'], dataset_ood.node_idx
