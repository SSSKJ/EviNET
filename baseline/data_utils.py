...
def evaluate_detect(model, dataset_ind, dataset_ood, criterion, eval_func, args, device):
    ...
    test_label = dataset_ind.y[test_idx]

    out = model(dataset_ind, device).cpu().detach()
    y = dataset_ind.y
    if args.method == "GPN" or args.method == 'SGCN':
        test_out = out[test_idx]
    else:
        test_out = torch.argmax(F.log_softmax(out[test_idx], dim=1), dim = 1)

    correct = (test_label.view(-1) == test_out.view(-1))

    aurc, _ = eval_aurc(correct, test_ind_score)
