...
class BetaProjection(nn.Module):
    def __init__(self, entity_dim, relation_dim, hidden_dim, num_layers):
        ...
        self.r_embedding = nn.Parameter(torch.zeros(self.relation_dim))
        nn.init.uniform_(
            tensor=self.r_embedding, 
            a=-self.embedding_range.item(), 
            b=self.embedding_range.item()
        )

    def forward(self, e_embedding):
        r_embedding = self.r_embedding.unsqueeze(0).expand(e_embedding.size(0), self.r_embedding.size(-1))
...
for run in range(args.runs):
    ...
    projector = BetaProjection(num_features, num_features//2, args.hidden_channels, 2).to(device)
    ...
    for epo in range(args.epochs):
        ...
        pbetaE = projector(betaE)
        pnode_alphas = betaE[:, :int(pbetaE.size(1)/2)]
        pnode_betas = betaE[:, int(pbetaE.size(1)/2):]
            
        train_idx = dataset_ind.splits['train']

        x_alpha = node_alphas[train_idx]
        x_beta = node_betas[train_idx]
        px_alpha = pnode_alphas[train_idx]
        px_beta = pnode_betas[train_idx]
        ...
        for class_idx in range(c):
            ...
            classN = deepset(torch.cat([px_alpha[y == class_idx], px_beta[y == class_idx]], dim=-1).unsqueeze(0)).squeeze(0)
        ...
        if eval_nodeE['test'][-1] > best_current:
            ...
            best_projector = projector.state_dict()
...
torch.save(best_projector, f'./models/{args.dataset}/{args.prefix}/{name}_projector.pth')
