...
from utils.calculation_tools import cal_logit, edl_loss, get_scoreNN_distance, cal_distance_raw
...
deepset.load_state_dict(torch.load(f'./models/{args.dataset}/{args.prefix}/{args.prefix}_h{args.hidden_channels}_l{args.lamda}_g{args.gamma}_lr{args.lr}_d{args.dropout}_a{args.aggr}_c{args.concat}_deepset.pth'))
encoder = GraphModel(dataset_ind.x.size(1), num_features, args).to(device)
encoder.load_state_dict(torch.load(f'./models/{args.dataset}/{args.prefix}/{args.prefix}_h{args.hidden_channels}_l{args.lamda}_g{args.gamma}_lr{args.lr}_d{args.dropout}_a{args.aggr}_c{args.concat}_encoder.pth'))
...
for run in range(args.runs):
    ...
    for epo in range(args.epochs):
        ...
        evidence = Beta2E(raw_distance).view(-1, class_alphas.size(0))
        ...
        for tag in ['train', 'valid', 'test']:
            ...
            raw_distance = cal_distance_raw(node_alphas[idx].unsqueeze(1), node_betas[idx].unsqueeze(1), class_alphas.unsqueeze(0), class_betas.unsqueeze(0)).view(-1, x_alpha.size(-1))
        ...
        test_ind_score_conflict, test_ind_score_vacuity = get_scoreNN_distance(node_alphas[dataset_ind.splits['test']], node_betas[dataset_ind.splits['test']], class_alphas, class_betas, Beta2E, args)

        # print(test_ind_score)
        _, test_ood_score_vacuity = get_scoreNN_distance(node_alphas[dataset_ood_te.node_idx], node_betas[dataset_ood_te.node_idx], class_alphas, class_betas, Beta2E, args)
        ...
        # except:
        #     print(test_ind_score_vacuity)
        #     print(test_ood_score_vacuity)
...
torch.save(best_Beta2E, f'./models/{args.dataset}/{args.prefix}/{name}_Beta2E.pth')
